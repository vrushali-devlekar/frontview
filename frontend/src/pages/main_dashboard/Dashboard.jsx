...
import { getProjects, getDashboardStats } from "../../api/api";
...
const StatusDot = ({ status }) => {
  ...
  return (
    <span
      className={`inline-block w-2 h-2 rounded-full shrink-0 ${colors[status] || "bg-[#52525b]"}`}
    />
  );
};
...
const StatusBadge = ({ status }) => {
  const map = {
    ...
    BUILDING: {
      cls: "bg-[#eab308]/10 text-[#eab308] border-[#eab308]/20",
      label: "Building",
    },
    SUCCESS: {
      cls: "bg-[#22c55e]/10 text-[#22c55e] border-[#22c55e]/20",
  ...
  const cfg = map[status] || {
    cls: "bg-white/5 text-[#71717a] border-white/10",
    label: status,
  };
  return (
    <span
...
const UptimeBar = () => {
  const data = [
    60, 70, 65, 80, 75, 90, 85, 95, 88, 92, 78, 82, 90, 95, 88, 92, 85, 90, 95,
    88, 92, 85, 90, 95, 88, 92, 85, 90, 95, 100,
  ];
  return (
...
const StatCard = ({
  ...
  <BentoCard hover={false} className="p-7 flex flex-col justify-between gap-6">
    ...
    <div>
      <p className="text-[30px] font-bold text-white tracking-tight leading-none">
        {value}
      </p>
      <div className="flex items-center justify-between mt-3">
        <p className="text-[12px] text-[#71717a] font-medium">{label}</p>
        <span
          className={`inline-flex items-center gap-0.5 text-[11px] font-bold ${isPositive ? "text-[#22c55e]" : "text-[#ef4444]"}`}
        >
          {isPositive ? (
  ...
  const stats = [
    {
      ...
      value: dbStats.totalProjects,
      delta: calculateDelta(
        parseInt(dbStats.totalProjects) || 0,
        parseInt(dbStats.previousProjects) || 0,
      ),
      isPositive:
        (parseInt(dbStats.totalProjects) || 0) >=
        (parseInt(dbStats.previousProjects) || 0),
      ...
      sparkData: dbStats.projectHistory || [
        5,
        7,
        6,
        9,
    ...
    {
      label: "Deployments",
      value: dbStats.totalDeployments,
      delta: calculateDelta(
        parseInt(dbStats.totalDeployments) || 0,
        parseInt(dbStats.previousDeployments) || 0,
      ),
      isPositive:
        (parseInt(dbStats.totalDeployments) || 0) >=
        (parseInt(dbStats.previousDeployments) || 0),
      ...
      sparkData: dbStats.deploymentHistory || [
        180,
        195,
        210,
        205,
    ...
    },
    {
      ...
      delta:
        calculateDelta(
          parseFloat(dbStats.successRate) || 0,
          parseFloat(dbStats.previousSuccessRate) || 0,
        ) + "%",
      isPositive:
        (parseFloat(dbStats.successRate) || 0) >=
        (parseFloat(dbStats.previousSuccessRate) || 0),
      ...
      sparkData: dbStats.successHistory || [
        97,
        98,
        97.5,
        98.5,
        ...
        99,
        98.9,
        parseFloat(dbStats.successRate) || 99.1,
      ],
      sparkColor: "#a855f7",
    },
    {
      label: "Avg Build",
      value: dbStats.avgBuildTime,
      delta:
        calculateDelta(
          parseInt(dbStats.avgBuildTime) || 0,
          parseInt(dbStats.previousAvgBuildTime) || 0,
        ) + "s",
      isPositive:
        (parseInt(dbStats.avgBuildTime) || 0) <=
        (parseInt(dbStats.previousAvgBuildTime) || 0),
      ...
      sparkData: dbStats.buildTimeHistory || [
        38,
        40,
        39,
        41,
        ...
        42,
        44,
        parseInt(dbStats.avgBuildTime) || 42,
      ],
      sparkColor: "#eab308",
    },
  ];

  const recentDeploys = dbStats.recentDeployments || [];

  const activityBars = dbStats.activityBars || [
            ...
            <div className="grid grid-cols-12 gap-5">
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-1.5 mt-1.5">
                            <GitBranch
                              size={10}
                              className="text-[#3f3f46] shrink-0"
                            />
                    ...
                    <BentoCard hover={false} className="h-full flex flex-col">
                      ...
                      <div className="flex flex-col divide-y divide-white/[0.04] flex-1">
                        {[
                          ...
                          >
                            <span className="text-[11px] font-mono text-[#3b82f6] truncate">
                              {k}
                            </span>
                            <span className="text-[11px] font-mono text-[#2d2d33] ml-2">
                              {v}
                            </span>
                          </div>
                        ))}
              ...
              <div className="col-span-12 mt-2">
                ...
                {!loading && projects.length === 0 && (
                  <BentoCard
                    hover={false}
                    className="flex flex-col items-center justify-center py-20 border-dashed mb-10"
                  >
                            ...
                            {[
                              {
                                icon: Terminal,
                                label: "Logs",
                                fn: () => navigate(`/deploy?projectId=${p.id}`),
                              },
                              {
                                icon: History,
                                label: "Deploys",
                              ...
                              },
                              {
                                ...
                                label: "Config",
                                fn: () =>
                                  navigate(`/settings?projectId=${p.id}`),
                              },
