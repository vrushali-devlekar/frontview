...
export default function NewProjectPage() {
  ...
  const handleImport = (repo) => {
    ...
    const selectedRepoName = repo?.name || String(repo?.fullName || "").split("/")[1];
    const cloneUrl = repo?.url
      ? `${String(repo.url).replace(/\/+$/, "")}.git`
      : `https://github.com/${owner}/${selectedRepoName}.git`;

    if (!owner || !selectedRepoName) {
      setError("Invalid repository payload. Please refresh and try again.");
      return;
    }

    setRepoInput(cloneUrl);
    setRepoName(`${owner}/${selectedRepoName}`);
    setBranch((repo.defaultBranch || "main").trim());
    setName(selectedRepoName.replace(/[^a-zA-Z0-9-]/g, '-').toLowerCase());
    setError("");
    setStep(2);
  };
  ...
  const handleCustomImport = () => {
    ...
    if (!name.trim()) {
      const repoSlug = parsed.repoName.split("/")[1] || "";
      setName(repoSlug.replace(/[^a-zA-Z0-9-]/g, "-").toLowerCase());
    }
    setError("");
    setStep(2);
  };
                      ...
                      <div className="space-y-8">
                        ...
                        <div className="pt-8">
                          <GlassButton 
                            ...
                            disabled={loading || !name.trim() || !repoInput.trim() || !repoName.trim()}
                          >
