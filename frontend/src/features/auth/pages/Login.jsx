...
const Login = () => {
  ...
  const { loginUser, isLoading, isError, isSuccess, message, resetAuth, user, loginWithGithub, token } = useAuth();
          ...
          <div className="bg-[#0d0f11] border border-white/10 p-6 md:p-8 shadow-2xl rounded-sm">
            ...
            <form className="space-y-4" onSubmit={onSubmit}>
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full bg-[#39ff14]/10 text-[#39ff14] border border-[#39ff14]/30 py-3 text-[11px] font-bold border-b-4 border-[#1a7a0a] hover:bg-[#39ff14]/20 transition-all uppercase tracking-widest mt-2 disabled:opacity-50"
              >
                {isLoading ? 'Logging in...' : 'Login to Panel'}
              </button>
            </form>
